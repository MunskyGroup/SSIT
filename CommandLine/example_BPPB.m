...
[sensSoln,bounds] = A.solve(FSPsoln.stateSpace);  % Solve the sensitivity problem
...
A.ssaOptions.Nexp = 150; A.ssaOptions.nSimsPerExpt = 200;
...
B{1} = A.loadData('ToggleSSAData50Expts.csv',{'x1','exp1_s1';'x2','exp1_s2'});
B{1}.pdoOptions.PDO = [];  % Do not use PDO.
B{2} = A.loadData('ToggleSSAData50Expts.csv',{'x1','exp1_s1_Distorted';'x2','exp1_s2_Distorted'});
B{2}.pdoOptions.PDO = [];  % Do not use PDO.
B{3} = A.loadData('ToggleSSAData50Expts.csv',{'x1','exp1_s1_Distorted';'x2','exp1_s2_Distorted'});
%% Plot Results
...
% fitErrorsB1 = B{1}.likelihoodSweep([2,3],linspace(.5,1.5,15),true);
% title('Ideal data. Original FSP.')
% fitErrorsB2 = B{2}.likelihoodSweep([2,3],linspace(.5,1.5,15),true);
% title('Binomial data distortion. Original FSP.')
% fitErrorsB3 = B{3}.likelihoodSweep([2,3],linspace(.5,1.5,15),true);
...
B{1}.fittingOptions.modelVarsToFit = [2,3];
B{2}.fittingOptions.modelVarsToFit = [2,3];
B{3}.fittingOptions.modelVarsToFit = [2,3];
options = optimset('Display','iter','MaxIter',100);
for mm = 1:A.ssaOptions.Nexp
    B{1} = B{1}.loadData('ToggleSSAData50Expts.csv',{'x1',['exp',num2str(mm),'_s1'];'x2',['exp',num2str(mm),'_s2']}); % Link non-distorted data.
    B{2} = B{2}.loadData('ToggleSSAData50Expts.csv',{'x1',['exp',num2str(mm),'_s1_Distorted'];'x2',['exp',num2str(mm),'_s2_Distorted']}); % Link non-distorted data.
    B{3} = B{3}.loadData('ToggleSSAData50Expts.csv',{'x1',['exp',num2str(mm),'_s1_Distorted'];'x2',['exp',num2str(mm),'_s2_Distorted']}); % Link non-distorted data.
    for m=1:3
        if mm==1; x0 = log10([A.parameters{A.fittingOptions.modelVarsToFit,2}]');
        else; x0 = log10(squeeze(MLE(m,:,mm-1))); end
        A = @(x)-B{m}.computeLikelihood(10.^x);  % We want to MAXIMIZE the likelihood.
        [x0,fMLE(m,:,mm)]  = fminsearch(A,x0,options);  MLE(m,:,mm) = 10.^x0;
    end; end
...
for m=1:3
    ...
    fimFreePars = FIM{m}(B{m}.fittingOptions.modelVarsToFit,B{m}.fittingOptions.modelVarsToFit);
