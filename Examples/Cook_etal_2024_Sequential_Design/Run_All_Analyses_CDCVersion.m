...
for Figure_to_Generate = 3
    switch Figure_to_Generate
        ...
        case 3  % Figure 3
            ...
            %%   Figure 3E,(Optimized Experiment Design)
            f2 = figure;
            set(f2,'Position',[939   687   404   238])
            colormap('sky')
            sz = size(finalExperimentDesignBFBD);
            ...
            % 
            % iRound = 7;
            % MAT = zeros(size(TestCasesBFBD.measurements{1}));
            % MAT(1,[1,7,14,21]) = 5;
            % for i = 1:iRound-1
            %     MAT = MAT + TestCasesBFBD.measurements{i};
            % end
            % 
            % for iExp = 1:size(MAT,1)
            %     for jT = 1:size(MAT,2)
            %         dat
            %     end
            % end
