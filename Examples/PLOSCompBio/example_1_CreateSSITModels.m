...
% paper): 
%STL1_4state.inputExpressions = {'Hog1','A*(((1-(exp(1)^(-r1*t)))*exp(1)^(-r2*t))/(1+((1-(exp(1)^(-r1*t)))*exp(1)^(-r2*t))/M))^n'};
STL1_4state.inputExpressions = {'Hog1','A*(((1-(exp(1)^(-r1*(t-100))))*exp(1)^(-r2*(t-100)))/(1+((1-(exp(1)^(-r1*(t-100))))*exp(1)^(-r2*(t-100)))/M))^n'};
...
STL1_4state.propensityFunctions = {...
          ...
          'kr1*s1';'k12*s1';'s2*3200*(k21o-(k21i*Hog1*((t-100)>0)))';...
