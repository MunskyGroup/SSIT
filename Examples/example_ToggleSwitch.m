...
A.customConstraintFuns = {'(lacI-3).^2.*(lambdaCI-3).^2'}; % Add FSP constraint
A.tSpan = [0:3:12];    % Set times at which to compute distributions
A.summarizeModel       % Print model summary to screen
A = A.formPropensitiesGeneral('ToggleModel'); % Generate codes model
