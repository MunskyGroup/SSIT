...
% It fills in the reactions table in the reactions tab.
        ...
        app.ReactionsTabOutputs.presetParameters = {4.7,0.0096, 0.13, 1, 0.024, 0.0083, 1.3e-4, 0.0023, 1.0, 0.0022, 0.0014};
        app.ReactionsTabOutputs.presetInputs = {'max(0,490*(1-(2.4*(9.3e9*(((1-exp(-(6.9e-5)*t))*exp(-(2.4e-3)*t))/(1+(((1-exp(-(6.9e-5)*t))*exp(-(2.4e-3)*t))/(6.4e-4))))^3.1))))'};
        ...
        app.ReactionsTabOutputs.modelInfo={'About the Model';'';'high-osmolarity glycerol (HOG) mitogen-activated protein kinase (MAPK) pathway in S. cerevisiae and its regulation of the HSP12 gene through different levels of osmotic shock.';'';'kij=conversion rate from state i to j';'kri=production rate of mRNA from state i';'kg=degradation rate'};
