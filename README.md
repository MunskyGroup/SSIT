...
- **Solve Models** using: 
  ...
  * Reduced order models (**Model Reduction**) by:
    ...
    - Reduce models using *Coarse Mesh Projections*
    - Reduce models using *Principle Orthogonal Decomposition*
* Create nested models using ``SSITMultiModel`` to associated related models with shared parameter sets to with different data sets
- **Compute Sensitivity** to parameter variations
- Compute the **Fisher Information Matrix** to determine the amount of parameter information available from chosen experiments
...
- **Maximum Likelihood Estimates** using gradient and non-gradient based searches
...
- Model and account for experimental and data processing **Measurement Noise**
  * Explore effects of **Extrinsic Noise** in parameters
  * Calibrate empirical **Probability Distortion Operators** (PDO) to quantify effects of data distortion, and include data distortion corrections in parameter estimation
- Inform and improve upon **Iterative Design of Experiment** (DoE) by searching an experiment design space for optimally informative experiments
