...
loadPrevious = false;
savedWorkspace = 'workspaceHog1Model';
...
switch iModel
    case 1
        % TODO - fix the propensities for case 1
        Model.propensityFunctions = {'k21*(gene==1)';
            'kr*(gene==0) + kr*(gene==1) + kr*(gene==2) + kr*(gene==3)'};
        Model.parameters = ({'k21',30;'kr',100;'g',0.005; ...
            'a0',0.01;'a1',1;'r1',0.4;'r2',.1});
        Model.fittingOptions.modelVarsToFit = 1:3;
    case 2
...
% TODO - Why does this line fail on first run, but run again and it goes??
...
varNames = {'Model',
    'pars'};
save('workspaceHog1Model',varNames{:})
...
params = cellfun(@(x) x, Model.parameters(:,2));
FIM_numeric = cell2mat(FIM);
FIMlog = diag(params) * FIM_numeric * diag(params);
...
%Model.fittingOptions.modelVarsToFit = 1:11;
numVars = length(Model.parameters);

MHOptions = struct('numberOfSamples',1000,'burnin',0,'thin',3);

log10PriorStd = 2*ones(1,numVars);
fim_withPrior = Model.evaluateExperiment(fimResults,Model.dataSet.nCells,...
        diag(log10PriorStd(1:numVars).^2));
FIMfree = fim_withPrior{1}(1:numVars,1:numVars);
...
MHOptions.useFIMforMetHast = false;
MHOptions.proposalDistribution=@(x)mvnrnd(x,covFree*proposalWidthScale);
...
%%
figNew = figure;
Model.plotMHResults(chainResults,[],'log',[],figNew)
...
Q = [3,4];
subplot(1,3,2)
Model.makeMleFimPlot(exp(chainResults.mhSamples)',FIM_numeric,Q,0.95,1); hold on
...
ModelSim = ModelSim.loadData('HogSSAData.csv',{'rna','exp1_s2'});
...
%% Calibrate the PDO for intensity data
% Read spot counts and intensity data
ModelPDOCytTot = Model.calibratePDO('filtered_data_2M_NaCl_Step.csv',...
    {'rna'},{'RNA_STL1_cyto_TS3Full'},{'RNA_STL1_total_TS3Full'},'AffinePoiss',true);
%ModelPDONucTot = Model.calibratePDO('filtered_data_2M_NaCl_Step.csv',...
%    {'rna'},{'RNA_STL1_nuc_TS3Full'},{'RNA_STL1_total_TS3Full'},'AffinePoiss',true);
ModelPDOIntens = Model.calibratePDO('filtered_data_2M_NaCl_Step.csv',...
    {'rna'},{'RNA_STL1_total_TS3Full'},{'STL1_avg_int_TS3Full'},'AffinePoiss',true);

%% FIM + Cyt PDO analyses
%  Analyze FIM with PDO for Cytoplasm data
fimsPDOIntens = ModelPDOCytTot.computeFIM([],'log');
%% FIM + Intensity PDO analyses
%  Analyze FIM with PDO for Intensity
fimsPDOIntens = ModelPDOIntens.computeFIM([],'log');
...
% First use nCellsOptDetA ('Determinant')
fimPDOIntensA = ModelPDOSpots.evaluateExperiment(fimsPDOIntens,nCellsOptDetA,diag(log10PriorStd.^2));

nCellsOptPDOintens = ModelPDOSpots.optimizeCellCounts(fimsPDOIntensA,nTotal,'tr[1:4]');

fimPDOIntensAvail = ModelPDOIntensEric.evaluateExperiment(fimsPDOIntensA,nCellsOptAvail,diag(log10PriorStd.^2));
figNew = figure; clf;
Model.plotMHResults(chainResults,[fimOpt,fimPDOSpots,fimTotal,fimPDOIntens],'log',[],figNew);
% figNew = figure; clf;
    % Model.plotMHResults(MHResults,[fimOpt,fimTotal,fimPDOIntensAvail],'log',[],figNew);
for i = 1:3
    for j = i:3
        subplot(3,3,(i-1)*3+j)
        CH = get(gca,'Children');
