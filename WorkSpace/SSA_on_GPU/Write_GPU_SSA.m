function Write_GPU_SSA(k,w,S,tprint,fun_name)
...
txt = ['function [X]=',fun_name,'(x0,N_run,N_split)\r\n'];
fprintf(fileID,txt);
...
fprintf(fileID,'parfor i = 1:N_split  %% Run loop over different GPU cards.\r\n');
for i=1:Nspec
    for j=1:Nt
        if i==1&&j==1
            txt = '  [x1_1';
...
for i=1:Nspec
    for j=1:Nt
        txt = ['  xp',num2str(i),'_',num2str(j),'(:,i)=gather(x',num2str(i),'_',num2str(j),');\r\n'];
%         txt = ['  X(',num2str(i),',',num2str(j),',:,i)=gather(x',num2str(i),'_',num2str(j),');\r\n'];
        fprintf(fileID,txt);
    end
end
...
fprintf(fileID,'for k=1:N_split\r\n');
for i=1:Nspec
    for j=1:Nt
        txt = ['  X(',num2str(i),',',num2str(j),',','(k-1)*n_run+1:k*n_run) = xp',num2str(i)','_',num2str(j),'(:,k);\r\n'];
        fprintf(fileID,txt);
    end
end
fprintf(fileID,'end\r\n');
