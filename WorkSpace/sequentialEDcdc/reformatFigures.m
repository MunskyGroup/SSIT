close all
clear all
[TestCasesBFBD,finalExperimentDesignBFBD] = sequentialExptDesignBatchRunner(1,1,1,true,false,[4]);
[TestCasesRD,finalExperimentDesignRD] = sequentialExptDesignBatchRunner(1,2,1,true,false,[4]);
%%
f = figure(101);
...
%% Values
clc
TestCasesRD.detCov(8)
% TestCasesRD.detFIMInv_Pred(7)
% TestCasesRD.stdDetFIMInv_Pred(7)
% TestCasesRD.detFIMTrueInv(8)

TestCasesBFBD.detCov(5)
% TestCasesBFBD.detFIMInv_Pred(4)
% TestCasesBFBD.stdDetFIMInv_Pred(4)
% TestCasesBFBD.detFIMTrueInv(5)
...
clear all
close all 
[TestCasesBFBD,finalExperimentDesignBFBD] = sequentialExptDesignBatchRunner(5,1,2,true,false,[4]);
[TestCasesRD,finalExperimentDesignRD] = sequentialExptDesignBatchRunner(5,2,2,true,false,[4]);
%%
f = figure(501);
set(f,'Position',[ 616   659   429   259])
...
for i = fs
    ...
    % ylabel('$\log_{10} (k_{\rm on}^{0})$','Interpreter','latex')
    % xlabel('$\log_{10} (k_{\rm off}^{0})$','Interpreter','latex')
...
clear all
close all 
[TestCasesBFBD,finalExperimentDesignBFBD] = sequentialExptDesignBatchRunner(8,1,2,true,true);
[TestCasesRD,finalExperimentDesignRD] = sequentialExptDesignBatchRunner(8,2,2,true,true);
%%
f = figure(801);
set(f,'Position',[ 616   659   429   259])
...
ModelTrue.parameters(:,2) = num2cell(TestCasesBFBD.pars(6,:));
% ModelTrue.parameters(:,2) = num2cell([0.00548871040192205          16.2459395123398        0.0159057121541332        0.0116719322252744       0.00454461004983959          11.7077637817397]);
...
ModelTrue = ModelTrue.loadData("ExampleData/Gated_dataframe_Ron_030624_NormalizedGR_bins.csv",...
    {'nucGR','normgrnuc';'cytGR','normgrcyt'},...
    {'Condition','GR_timesweep';'Dex_Conc','10'});

%% Make and format figures.
close all
