classdef Propensity
    ...
    methods (Access = public, Static)
        ...
        function obj = createAsHybridVec(symbolicExpression, stoichMatrix, nonXTpars, species, upstreamODEs, logicTerms)
            arguments
                symbolicExpression
                stoichMatrix
                nonXTpars
            ...
            for i = 1:n_reactions
                ...
                obj{i} = ssit.Propensity(stoichMatrix(jStochastic,i), i);
                obj{i}.ODEstoichVector = stoichMatrix(jODE,i);

                if ~isempty(jODE)&&max(abs(obj{i}.stoichVector))~=0&&max(abs(obj{i}.ODEstoichVector))~=0
                    warning(['Reaction ',num2str(i),' changes both ODE and stochastic species. Removing effect on upstream species.'])
                    obj{i}.ODEstoichVector = 0*obj{i}.ODEstoichVector;
                end
                prop_vars = symvar(symbolicExpression{i});

                syms(prop_vars);
                syms t
                expr_tx = symbolicExpression{i};
                ...
                if sum(Jx)==0
                    % No x dependance
                    factors = [str2sym('1'),symbolicExpression{i}];
                else
                    % Determine if the symbolicExpression is isFactorizable
                    factors = factor(symbolicExpression{i}, prop_vars(Jx));
                    factors = [prod(factors(2:end)),factors(1)];
                    ...
                    for i2 = 2:length(factors)
                        ...
                        Jx = contains(TMP,speciesStoch);
                        if (sum(Jx) > 1)
                            obj{i}.isFactorizable = false;
                            break;
                        end
                    end
                end

                if (obj{i}.isFactorizable)
                    % If the propensity is isFactorizable, collect the
                    % time-dependent and state-dependent function handles
                    ...
                    hybridFactor = sym2propfun(expr_t, true, false, nonXTpars(:,1), speciesStoch, varODEs, logicTerms{i});
                    stateDependentFactor = sym2propfun(expr_x, false, true, nonXTpars(:,1), speciesStoch, [], logicTerms{i});
                                      
                    if ~isempty(jODE)
                        obj{i}.isTimeDependent = true;
                    ...
                    if sign(TmpHybridFactor)==-1
                        obj{i}.(timeFunName) = sym2propfun(-expr_t, true, false, nonXTpars(:,1), speciesStoch, varODEs, logicTerms{i});
                        obj{i}.stateDependentFactor =  sym2propfun(-expr_x, false, true, nonXTpars(:,1), speciesStoch, [], logicTerms{i});
                        expr_t_vec(i) = -expr_t;
                    else
                        obj{i}.(timeFunName) = hybridFactor;
                        obj{i}.stateDependentFactor =  stateDependentFactor;
                        expr_t_vec(i) = expr_t;
                    end
                    if ismember(t,symvar(expr_t))||isfield(logicTerms{i},'logT')&&~isempty(logicTerms{i}.logT)
                        obj{i}.isTimeDependent = true;
                    end
                else
                    for i2=1:length(upstreamODEs)
                    ...
                    obj{i}.(jntFactorName) = sym2propfun(expr_tx, true, true, nonXTpars(:,1), speciesStoch, varODEs, logicTerms);
                    obj{i}.isTimeDependent = true;
                end
