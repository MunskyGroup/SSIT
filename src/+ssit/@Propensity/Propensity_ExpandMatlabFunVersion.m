            ...
            for iRxn = 1:n_reactions
                ...
                if (obj{iRxn}.isFactorizable)
                    ...
                    if obj{iRxn}.anonymousT
                        [obj{iRxn}.(timeFunName),expr_dt_vec_dodei] = ... % sym2propfun(signHybridFactor*expr_t, true, false, nonXTpars(:,1), speciesStoch, varODEs, logicTerms(iRxn), true);
...
elseif (time_dep)
    ...
    exprHandle = str2func([fhandle_var exprStr]);
    % t = sym('t','real');
    % x = sym('x',[length(species),1],'real');
    % Parameters = sym("Parameters",[length(nonXTpars),1],'positive');
    % exprStr2 = (simplify(eval(exprStr)));
...
for i=1:length(logicTerms)
    ...
    if isfield(logicTerms{i},'logT')
        for j=1:size(logicTerms{i}.logT,1)
            % exprStr=strrep(exprStr,logicTerms{i}.logT{j,2},logicTerms{i}.logT{j,1});
            % exprStr=regexprep(exprStr,['\<',logicTerms{i}.logT{j,2},'\>'],logicTerms{i}.logT{j,1});
            symbolicExpression = subs(symbolicExpression,logicTerms{i}.logT{j,2},eval(logicTerms{i}.logT{j,1}));
        end
    end
    if isfield(logicTerms{i},'logX')
        %             state_dep = true;
        for j=1:size(logicTerms{i}.logX,1)
            % exprStr=strrep(exprStr,logicTerms{i}.logX{j,2},logicTerms{i}.logX{j,1});
            % exprStr=regexprep(exprStr,['\<',logicTerms{i}.logX{j,2},'\>'],logicTerms{i}.logX{j,1});
            symbolicExpression = subs(symbolicExpression,logicTerms{i}.logX{j,2},eval(logicTerms{i}.logX{j,1}));
        end
    end
    if isfield(logicTerms{i},'logJ')
        %             time_dep = true;
        ...
        for j=1:size(logicTerms{i}.logJ,1)
            % exprStr=strrep(exprStr,logicTerms{i}.logX{j,2},logicTerms{i}.logX{j,1});
            % exprStr=regexprep(exprStr,['\<',logicTerms{i}.logX{j,2},'\>'],logicTerms{i}.logX{j,1});
            symbolicExpression = subs(symbolicExpression,logicTerms{i}.logJ{j,2},eval(logicTerms{i}.logJ{j,1}));
        end
    end
end
...
ifn = numel(files) + 1;
fn = [pathToPropensityFuns, filesep, prefixName,'_fun_',num2str(ifn),'.m'];
...
if writeFiles
    if isempty(varODEs)
        if (time_dep && state_dep)
            exprHandle = matlabFunction(symbolicExpression,'Vars',{t,states,parameters},'File',fn,'Sparse',useSparse);
        elseif time_dep
            exprHandle = matlabFunction(symbolicExpression,'Vars',{t,parameters},'File',fn,'Sparse',useSparse);
        else
            exprHandle = matlabFunction(symbolicExpression,'Vars',{states,parameters},'File',fn,'Sparse',false);
        end
    else
        if (time_dep && state_dep)
            exprHandle = matlabFunction(symbolicExpression,'Vars',{t,states,parameters,varODEs},'File',fn,'Sparse',useSparse);
        elseif time_dep
            exprHandle = matlabFunction(symbolicExpression,'Vars',{t,parameters,varODEs},'File',fn,'Sparse',useSparse);
        else
            exprHandle = matlabFunction(symbolicExpression,'Vars',{states,parameters,varODEs},'File',fn,'Sparse',useSparse);
        end
    end
