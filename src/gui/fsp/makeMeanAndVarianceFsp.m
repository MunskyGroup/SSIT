...
for iSpecies = 1:Nd
    Plts_to_make(iSpecies) = max(contains(app.SpeciestoShowListBoxMeans.Value,app.ReactionsTabOutputs.varNames{iSpecies}));
end
cols = {'b','r','g','m','c','y','k'};
...
for iplt=1:Nd
    if Plts_to_make(iplt)
        ...
        if app.FspMeanVarShowOdeCheckBox.Value == 1
            ...
            LG{end+1} = [append(char(app.NameTable.Data(iplt,2)),' ode')];
            LG{end+1} = [char(app.NameTable.Data(iplt,2))];
        end
    end
end
